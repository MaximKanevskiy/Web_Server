            ...
            <body>
		        <form method="post" action="/login" accept-charset="UTF-8">
				    <div class="mainContainer">
						...
						<input type="text" autocomplete="off" placeholder="Введите фамилию" name="surname" required>
						<label for="password"></label>
                        <input type="password" autocomplete="off" placeholder="Пароль админа" name="password" required>
...
void homeHandler(const Request& req, Response& res)
{
    ...
    if (req.method == "GET")
    {
        ...
        page += u8"        alert('Файл успешно загружен!');";
        page += u8"    }";
        page += u8"};";
        page += u8"</script>";
        page += u8"<body><div><h1>Таблица пользователей</h1>";
        page += u8"<table><tr><td><strong>Фамилия</strong></td><td><strong>Группа</strong></td><td><strong>Роль</strong></td></tr>";
        ...
        for (const auto& user : users)
        {
            ...
            page += u8"<input type='hidden' name='surname' value='" + user.surname + "'>";
            page += u8"<button class='delete-button' type='submit'>Удалить</button>";
            page += u8"</form>";
            page += u8"</td></tr>";
        }
        ...
        page += u8"<button class='logout-button' type='submit'>Выход</button>";
        page += u8"</form>";
        page += u8"</div>";
        page += u8"<div>";
        ...
        page += u8"<button type='sumbit'>Отправить данные</button>";
        page += u8"</form>";
        page += u8"</div>";
        page += u8"</body>";
        page += u8"</html>";
...
{
    httplib::Client client("192.168.253.55", 8090);
